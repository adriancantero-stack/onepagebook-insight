-- Update the search function to be more permissive and return more results
...
BEGIN
  ...
  WHERE 
    ...
    AND (
      ...
      -- Also match if any word in the search matches
      OR normalize_text(b.title) ~ normalize_text(search_query)
      OR normalize_text(b.author) ~ normalize_text(search_query)
    )
  ORDER BY 
