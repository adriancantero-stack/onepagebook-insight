...
const getEmailContent = (language: string, userName: string): WelcomeEmailContent => {
  const contents: Record<string, WelcomeEmailContent> = {
    ...
    es: {
      ...
      step3Desc: "Lee, escucha en audio y pon las ideas en práctica. ¡Gana XP con cada acción!",
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    const { email, userName = "Usuário", language = "pt" } = await req.json();

    if (!email) {
      throw new Error("Email is required");
    }

    console.log(`📧 Sending test welcome email to: ${email} (${language})`);

    // Get email content
    const content = getEmailContent(language, userName);
